...
class FrequencyBand(IntEnum):  # Enum keys are frequency ranges in Hz
    ...
    _1450_3500 = 2
    _3500_5500 = (
        3  # This one (3.5 KHz - 5.5 KHz) should not be used in legacy mode
    )
...
class FrequencyPeak:
    ...
    def get_frequency_hz(self) -> float:
        ...
        return self.corrected_peak_frequency_bin * (
    ...
    def get_amplitude_pcm(self) -> float:
        ...
        return (
            sqrt(exp((self.peak_magnitude - 6144) / 1477.3) * (1 << 17) / 2)
            / 1024
        )
...
class DecodedMessage:
    ...
    def decode_from_uri(cls, uri: str):
        ...
        return cls.decode_from_binary(
            b64decode(uri.replace(DATA_URI_PREFIX, '', 1))
        )
    ...
    def encode_to_binary(self) -> bytes:
        ...
        for frequency_band, frequency_peaks in sorted(
            ...
            for frequency_peak in frequency_peaks:
                ...
                peaks_buf.write(
                    (frequency_peak.corrected_peak_frequency_bin).to_bytes(
                        2, 'little'
                    )
                )
            ...
            contents_buf.write(
                (0x60030040 + int(frequency_band)).to_bytes(4, 'little')
            )
    ...
    def encode_to_uri(self) -> str:
        ...
        return DATA_URI_PREFIX + b64encode(self.encode_to_binary()).decode(
            'ascii'
        )
