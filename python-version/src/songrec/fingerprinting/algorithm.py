...
from songrec.fingerprinting.signature_format import (
    DecodedMessage,
    FrequencyPeak,
    RawSignatureHeader,
    FrequencyBand,
...
class RingBuffer(list):
    def __init__(self, buffer_size: int, default_value: Any = None):
        ...
        if default_value is not None:
            list.__init__(
                self, [copy(default_value) for position in range(buffer_size)]
            )
        else:
...
class SignatureGenerator:
    def __init__(self):
        ...
        self.input_pending_processing: List[
            int
        ...
        )  # Lists of 1025 floats, premultiplied with a Hanning function before being passed through FFT, computed from the ring buffer every new 128 samples

        self.spread_ffts_output: RingBuffer[List[float]] = RingBuffer(
    ...
    def get_next_signature(self) -> Optional[DecodedMessage]:
        ...
        ) - self.samples_processed >= 128 and (
            self.next_signature.number_samples
            / self.next_signature.sample_rate_hz
    ...
    def do_fft(self, batch_of_128_s16le_mono_samples):
        ...
        self.ring_buffer_of_samples.position += len(
            batch_of_128_s16le_mono_samples
        )
        self.ring_buffer_of_samples.position %= 2048
        self.ring_buffer_of_samples.num_written += len(
            batch_of_128_s16le_mono_samples
        )
        ...
        assert len(fft_results) == 1025 and len(
            excerpt_from_ring_buffer
        ) == 2048 == len(HANNING_MATRIX)
    ...
    def do_peak_spreading(self):
        ...
        origin_last_fft: List[float] = self.fft_outputs[
            self.fft_outputs.position - 1
        ]
            ...
            if (
                ...
                if fft_minus_46[bin_position] > max_neighbor_in_fft_minus_49:
                        ...
                        *range(214, 250, 7),
                        ...
                        max_neighbor_in_other_adjacent_ffts = max(
                            self.spread_ffts_output[
                                (
                                    self.spread_ffts_output.position
                                    + other_offset
                                )
                                % self.spread_ffts_output.buffer_size
                            ][bin_position - 1],
                    ...
                    if (
                        ...
                        peak_magnitude = (
                            log(max(1 / 64, fft_minus_46[bin_position]))
                            * 1477.3
                            + 6144
                        )
                        peak_magnitude_before = (
                            log(max(1 / 64, fft_minus_46[bin_position - 1]))
                            * 1477.3
                            + 6144
                        )
