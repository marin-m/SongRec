...
for version in bionic focal groovy hirsute; do
    ...
    cp -ra ../../ "${temp_dir}/songrec-0.1.3${version}"

    cd "${temp_dir}/songrec-0.1.3${version}"
    ...
    dput ppa:marin-m/songrec "../../songrec_0.1.3${version}_source.changes"

    cd "${ORIG_DIR}"

    rm -rf "${temp_dir}/songrec-0.1.3${version}"
