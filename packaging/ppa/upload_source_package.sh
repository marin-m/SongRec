...
for version in focal jammy noble questing resolute; do

    rm -rf ../../target/ ../../vendor/ ../../.flatpak-builder ../flatpak/.flatpak-builder ../../repo ../../.cargo

    cp -ra ../../ "${temp_dir}/songrec-0.5.0+1${version}"

    cd "${temp_dir}/songrec-0.5.0+1${version}"
    ...
    dput ppa:marin-m/songrec "../../songrec_0.5.0+1${version}_source.changes"

    cd "${ORIG_DIR}"

    rm -rf "${temp_dir}/songrec-0.5.0+1${version}"
