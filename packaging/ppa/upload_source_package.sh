...
for version in focal jammy noble questing resolute; do
    ...
    cp -ra ../../ "${temp_dir}/songrec-0.5.0+2${version}"

    cd "${temp_dir}/songrec-0.5.0+2${version}"
    ...
    dput ppa:marin-m/songrec "../../songrec_0.5.0+2${version}_source.changes"

    cd "${ORIG_DIR}"

    rm -rf "${temp_dir}/songrec-0.5.0+2${version}"
