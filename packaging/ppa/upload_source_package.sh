...
for version in bionic focal jammy lunar mantic noble; do
    ...
    cp -ra ../../ "${temp_dir}/songrec-0.4.0+1${version}"

    cd "${temp_dir}/songrec-0.4.0+1${version}"
    ...
    dput ppa:marin-m/songrec "../../songrec_0.4.0+1${version}_source.changes"

    cd "${ORIG_DIR}"

    rm -rf "${temp_dir}/songrec-0.4.0+1${version}"
